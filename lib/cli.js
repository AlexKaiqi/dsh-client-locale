#!/usr/bin/env node
import { createPseudoCatalog, messageSignature, messageSourceHash, sourceCatalogHash, validateTranslationCatalog } from "./tooling.js";
...
function sourceCatalog(value, path) {
	if (!isRecord(value) || value.schemaVersion !== 1 || typeof value.namespace !== "string" || typeof value.sourceLocale !== "string" || !isRecord(value.messages)) throw new Error(`${path} is not a DSH source catalog`);
	const messages = {};
	for (const [id, entry] of Object.entries(value.messages)) {
		if (!isRecord(entry) || typeof entry.defaultMessage !== "string") throw new Error(`${path}: message "${id}" has no defaultMessage`);
		const descriptor = {
			id,
		...
		if (isRecord(entry.start) && typeof entry.start.line === "number" && typeof entry.start.column === "number") descriptor.start = {
			line: entry.start.line,
			column: entry.start.column,
			...typeof entry.start.offset === "number" ? { offset: entry.start.offset } : {}
		};
		if (isRecord(entry.end) && typeof entry.end.line === "number" && typeof entry.end.column === "number") descriptor.end = {
			line: entry.end.line,
...
function translationCatalog(value, path) {
	if (!isRecord(value) || value.schemaVersion !== 1 || typeof value.namespace !== "string" || typeof value.sourceLocale !== "string" || typeof value.locale !== "string" || typeof value.sourceHash !== "string" || !isRecord(value.sourceHashes) || !isRecord(value.messages) || typeof value.generatedAt !== "string" || ![
		"machine",
		"mixed",
		"reviewed"
	].includes(String(value.reviewState))) throw new Error(`${path} is not a DSH translation catalog`);
	const messages = {};
	...
	const locked = Array.isArray(value.locked) ? value.locked.filter((key) => typeof key === "string") : void 0;
	return {
		schemaVersion: 1,
		namespace: value.namespace,
		sourceLocale: value.sourceLocale,
...
async function extractCommand(args) {
	...
	try {
		const packagePath = createRequire(import.meta.url).resolve("@formatjs/cli/package.json");
		...
		if (result.status !== 0) throw new Error(result.stderr.trim() || result.stdout.trim() || "FormatJS extraction failed");
...
async function pseudoCommand(args) {
	const sourcePath = required(args.options, "source");
	const outFile = required(args.options, "out-file");
	const localeValue = args.options.get("locale") ?? "en-XA";
	if (localeValue !== "en-XA" && localeValue !== "ar-XB") throw new Error("--locale must be en-XA or ar-XB");
	const source = sourceCatalog(await readJson(sourcePath), sourcePath);
...
async function translateLocale(source, locale, outPath, endpoint) {
	let existing;
	try {
	...
	if (existing && (existing.namespace !== source.namespace || existing.locale !== locale)) throw new Error(`${outPath} belongs to another namespace or locale`);
	const locked = new Set((existing?.locked ?? []).filter((key) => source.messages[key] !== void 0));
	const requested = {};
	...
	if (Object.keys(requested).length > 0) {
		...
		for (const key of Object.keys(requested)) if (messages[key] === void 0) throw new Error(`translator omitted key "${key}"`);
		response = {
			messages,
			...typeof value.provider === "string" ? { provider: value.provider } : {}
		};
	...
	for (const key of Object.keys(source.messages)) {
		...
		if (existingMessage !== void 0) messages[key] = existingMessage;
		const existingHash = existing?.sourceHashes[key];
		if (existingHash !== void 0) sourceHashes[key] = existingHash;
	}
	Object.assign(messages, response.messages);
	for (const [key, descriptor] of Object.entries(source.messages)) if (!locked.has(key) || response.messages[key] !== void 0) sourceHashes[key] = messageSourceHash(key, descriptor);
	const catalog = {
		schemaVersion: 1,
		namespace: source.namespace,
		sourceLocale: source.sourceLocale,
	...
	const blockingIssues = validateTranslationCatalog(source, catalog).filter((issue) => !(issue.code === "stale-source" && locked.has(issue.key)));
	if (blockingIssues.length > 0) throw new Error(blockingIssues.map((issue) => `${issue.code} ${issue.key}: ${issue.message}`).join("\n"));
	...
	for (const key of locked) {
		...
		if (descriptor && sourceHashes[key] !== messageSourceHash(key, descriptor)) process.stderr.write(`${outPath}: locked translation "${key}" needs review after its source changed\n`);
	}
}
async function translateCommand(args) {
	...
	const source = sourceCatalog(await readJson(sourcePath), sourcePath);
	for (const locale of locales) await translateLocale(source, locale, join(outDir, `${locale}.json`), endpoint);
}
function usage() {
