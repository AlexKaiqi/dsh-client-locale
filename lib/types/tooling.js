...
export function messageSourceHash(id, descriptor) {
...
export function validateTranslationCatalog(source, translation) {
    const issues = [];
...
function transformLiterals(elements) {
    for (const element of elements) {
        switch (element.type) {
            ...
            case TYPE.plural:
                for (const option of Object.values(element.options))
                    transformLiterals(option.value);
                break;
            case TYPE.tag:
...
export function createPseudoCatalog(source, locale = 'en-XA', generatedAt = new Date().toISOString()) {
    const messages = {};
    const sourceHashes = {};
