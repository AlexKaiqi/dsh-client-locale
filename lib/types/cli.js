...
function sourceCatalog(value, path) {
    ...
    for (const [id, entry] of Object.entries(value.messages)) {
        ...
        const descriptor = { id, defaultMessage: entry.defaultMessage };
...
function translationCatalog(value, path) {
    ...
    for (const [key, message] of Object.entries(value.messages)) {
        if (typeof message !== 'string')
            throw new Error(`${path}: translation "${key}" is not a string`);
        messages[key] = message;
    }
    const sourceHashes = {};
    for (const [key, hash] of Object.entries(value.sourceHashes)) {
        if (typeof hash !== 'string')
            throw new Error(`${path}: source hash "${key}" is not a string`);
        sourceHashes[key] = hash;
    }
    const locked = Array.isArray(value.locked)
        ? value.locked.filter((key) => typeof key === 'string')
        : undefined;
    return {
        schemaVersion: 1,
        namespace: value.namespace,
...
async function extractCommand(args) {
    ...
    if (args.positionals.length === 0)
        throw new Error('extract requires at least one source glob');
    ...
    try {
        ...
        if (!isRecord(extracted))
            throw new Error('FormatJS returned an invalid message map');
        const messages = {};
        for (const [id, entry] of Object.entries(extracted)) {
            if (!isRecord(entry) || typeof entry.defaultMessage !== 'string')
                continue;
            messages[id] = { ...entry, id };
        }
        const source = { schemaVersion: 1, namespace, sourceLocale, messages };
...
async function checkCommand(args) {
    ...
    for (const path of args.positionals) {
        ...
        for (const issue of issues)
            process.stderr.write(`${path}: ${issue.code}: ${issue.key}: ${issue.message}\n`);
        issueCount += issues.length;
    }
    if (issueCount > 0)
        throw new Error(`${issueCount} catalog issue(s) found`);
...
async function translateLocale(source, locale, outPath, endpoint) {
    ...
    for (const [key, descriptor] of Object.entries(source.messages)) {
        ...
        if (locked.has(key) || existing?.sourceHashes[key] === currentHash)
            continue;
        requested[key] = {
    ...
    if (Object.keys(requested).length > 0) {
        ...
        if (!result.ok)
            throw new Error(`translator returned HTTP ${result.status}`);
        ...
        for (const [key, translated] of Object.entries(value.messages)) {
            if (!(key in requested))
                throw new Error(`translator returned unexpected key "${key}"`);
            if (typeof translated !== 'string')
                throw new Error(`translator returned non-string key "${key}"`);
            messages[key] = translated;
        }
        for (const key of Object.keys(requested)) {
            if (messages[key] === undefined)
                throw new Error(`translator omitted key "${key}"`);
        }
    ...
    for (const key of Object.keys(source.messages)) {
        ...
        if (existingMessage !== undefined)
            messages[key] = existingMessage;
        const existingHash = existing?.sourceHashes[key];
        if (existingHash !== undefined)
            sourceHashes[key] = existingHash;
    }
...
async function main() {
    ...
    if (command === 'extract')
        return extractCommand(args);
    if (command === 'check')
        return checkCommand(args);
    if (command === 'pseudo')
        return pseudoCommand(args);
